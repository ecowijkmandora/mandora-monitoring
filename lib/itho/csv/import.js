...
importCsvEnergyInstallation = (location, filename, parser) => {
	...
	csvEnergyInstallationParser.on('error', err => {
		logger.error(
			`Error occured while running CSV importy (importCsvEnergyInstallation) for location ${location}`,
			err.message
		)
	})
...
importCsvTemperatureBoiler = (location, filename, parser) => {
	...
	csvTemperatureBoilerParser.on('error', err => {
		logger.error(
			`Error occured while running CSV importy (importCsvTemperatureBoiler) for location ${location}`,
			err.message
		)
	})
