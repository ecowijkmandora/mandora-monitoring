...
importCsvEnergy = (location, buffer) => {
	...
	csvEnergyParser.on('readable', () => {
		...
		while ((record = csvEnergyParser.read())) {
			if (record.length != ITHO_CSV_MEASUREMENT_ENERGY_COLUMNS) {
				logger.warn(
			...
			_.forEach(ITHO_CSV_MEASUREMENT_ENERGY_FIELDS, fieldset => {
				...
				_.forEach(fieldset, (field, idx, arr) => {
					...
					logger.debug(`* importCsvEnergy for location ${location}: Field value (${idx}) ${field}:`, value)
				...
				if (isValid) {
					...
					console.warn(
						`! importCsvEnergy for location ${location}: Ignoring fieldset because of empty value`,
						record
					)
				}
			})
	...
	csvTemperatureParser.on('readable', () => {
		...
		while ((record = csvTemperatureParser.read())) {
			...
			_.forEach(ITHO_CSV_MEASUREMENT_TEMPERATURE_FIELDS, fieldset => {
				...
				_.forEach(fieldset, (field, idx, arr) => {
					let value = record[idx]
					logger.debug(`* importCsvTemperature for location ${location}: Field value (${idx}) ${field}:`, value)
				...
				if (isValid) {
					...
					console.warn(
						`! importCsvTemperature for location ${location}: Ignoring fieldset because of empty value`,
						record
					)
				}
			})
